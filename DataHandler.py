...
class DataControlWidget(QObject):
    ...
    def onMouseClicked(self, point):
        ...
        self.wlen_range = self.avg_wl[wmin_idx:wmax_idx]
        ...
        self.time_range = self.tl_t0[tmin_idx:tmax_idx]
        ...
        if self.ui.fft_checkbox.isChecked():
            ...
            exp_result = least_squares(fit_trace, 
                                       x0=[0.1, 1.0, 100, -0.5, 0.25, 0.125],
                                       bounds=([0.01, 0.1, 1.0, -np.inf, -np.inf, -np.inf], [1.0, 1e2, 1e4, np.inf, np.inf, np.inf]),
                                       args=(self.time_range, self.kinetics_trace),
                                       method='trf',
                                       tr_solver='lsmr',
                                       loss='soft_l1',
                                       jac='3-point',
            ...
            self.parent.traces_widget.spectral.plot(self.wlen_range, self.spectrum_trace, pen='b', name='<math>&Delta;t = {} ps<math>'.format(round(self.x,2)))

        else:
            self.parent.traces_widget.kinetics.setLabel('left', '<math>&Delta;A</math>', units='OD', unitPrefix='m', **labelstyle)
            self.parent.traces_widget.kinetics.setLabel('bottom', 'Time', units='ps', **labelstyle)
            self.parent.traces_widget.kinetics.plot(self.time_range, self.kinetics_trace, pen='b', name='<math>&lambda; = {} nm</math>'.format(round(self.y,2)))
            self.parent.traces_widget.spectral.plot(self.wlen_range, self.spectrum_trace, pen='b', name='<math>&Delta;t = {} ps<math>'.format(round(self.x,2)))
    ...
    def getTraces(self):
        try:
            self.parent.compare_traces_widget.ccount
        except:
            self.parent.compare_traces_widget.ccount = 0
        finally:
            if self.parent.compare_traces_widget.ccount < 10:
                pen = {'color':colors_list[self.parent.compare_traces_widget.ccount], 'width':2}
                self.parent.compare_traces_widget.ccount += 1
            else:
                self.parent.compare_traces_widget.ccount = 0
                pen = {'color':colors_list[self.parent.compare_traces_widget.ccount], 'width':2}

            dataname = self.parent.controls_widget.dataname_input.text()
            self.parent.compare_traces_widget.kinetics.setTitle(dataname)
            self.parent.compare_traces_widget.spectral.setTitle(dataname)
            self.parent.compare_traces_widget.kinetics.plot(self.time_range, self.kinetics_trace, pen=pen, name='<math>&lambda; = {} nm</math>'.format(round(self.y,2)))
            self.parent.compare_traces_widget.spectral.plot(self.wlen_range, self.spectrum_trace, pen=pen, name='<math>&Delta;t = {} ps<math>'.format(round(self.x,2)))
    ...
    def exportTraces(self):
        try:
            ...
            self.parent.parent.traces_widget.kinetics.plot(self.time_range, self.kinetics_trace/self.kinetics_trace.max(), pen=pen, name=dataname)
            self.parent.parent.traces_widget.spectral.plot(self.wlen_range, self.spectrum_trace/self.spectrum_trace.max(), pen=pen, name=dataname)


    def plotTraces(self):
        if self.parent.compare_traces_widget.isVisible():
            self.parent.compare_traces_widget.hide()
        else:
            self.parent.compare_traces_widget.show()
