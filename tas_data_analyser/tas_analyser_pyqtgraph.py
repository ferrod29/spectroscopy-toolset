...
def exp_decay(x, p):
    '''Exponential decay of population. This function called for evaluation after fitting'''
    return p[2]*np.exp(-x/p[0]) + p[3]*np.exp(-x/p[1]) + p[4]

def exp_decay_fit(p, x, y):
    '''Exponential decay of population. This function called for the nonlinear fitting engine'''
    return exp_decay(x, p) - y

def vib_decay(x, p):
    '''Vibrational coherence function.  This function called for the nonlinear fitting engine'''
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.newGraphAct.setStatusTip("Open a new control dataset window")
    ...
    def plotterWindow(self):
        ...
        '''
        plotter_window = TAS_Analiser(self)
...
class PlotsWidget(QMainWindow):
    def __init__(self, parent=None):
        ...
        # self.loadAct.setStatusTip('Load entire data file')
        #
        # self.stderrChecked = QAction('StdErr', self, checkable=True)
        # self.stderrChecked.setChecked(False)
        # self.fitChecked = QAction('Fit', self, checkable=True)
        # self.fitChecked.setChecked(False)
        # self.fftChecked = QAction('FFT', self, checkable=True)
        # self.fftChecked.setChecked(False)
        #
        # self.menubar = self.menuBar()
        ...
        # self.toolbar.addAction(self.stderrChecked)
        # self.toolbar.addAction(self.fitChecked)
        # self.toolbar.addAction(self.fftChecked)
...
class TAS_Analiser(QMainWindow):
    ...
    def loadData(self):
        ...
        if len(self.files_list)>0:
            ...
            self.plotswidget.statusbar.showMessage('Succeeded to load datafiles')

        else:
            self.plotswidget.statusbar.showMessage('Failed to load any datafile')
            pass
    ...
    def correctData(self):
        ...
        self.writeUI(self.delta_wl, self.W0, wlen_min, wlen_max, self.T0, time_min, time_max, inty_min, inty_max)
        self.plotData()
        self.plotswidget.statusbar.showMessage('Data have been corrected: smooth={}, chirp={}'.format(smooth, chirp))
